"""P1 classifier: choose a simple task tier and execution route.

The classifier is intentionally small. It demonstrates how a gate can classify
requests before generation without introducing provider-specific dependencies.
    ...
    def classify(self, text: str, risk_level: str = "low") -> P1Routing:
        ...
        if has_space:
            return P1Routing(
                ...
                reason="长周期/跨公司/基础设施级任务，匹配高成本稀缺算力。",
            )
        if has_trivial:
            return P1Routing(
                tier="P3", compute="low", cost_tier="low",
                route="local", worth_it=True,
                reason="低价值短任务，本地轻量模型即可，不应占用高成本大模型算力。",
            )
        # 标准任务：地面推理
